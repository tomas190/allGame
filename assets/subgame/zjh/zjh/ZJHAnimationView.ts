...
export default class ZJHAnimationView extends cc.Component {
    ...
    getDealSequence(seats,Index:number){
        let tempList = [];
        tempList.push(Index);//先加入庄家
        while(tempList.length < seats.length){
            Index = Index >= 4 ? 0 : Index+1;
            for(let i=0; i< seats.length; ++i){
                if(seats[i] === Index){//当前座位号存在
                    tempList.push(Index);
                }
            }
        }
    ...
    dealCard(data){
        let len =  data.betseat.length;
        let totalPoker = len*3;//总共发多少张牌
        let tcount = totalPoker;
        this.parent.setBanker(data.zhuang);
        let userList = this.getDealSequence(data.betseat,data.zhuang);
        for(let i=0; i<totalPoker;++i){
            ...
            let call1 = cc.callFunc(()=>{//发牌音效
                Notification.sendNotify(Event.EVENT_MUSIC_EFFECT,{tag:4,mid:8,isLoop:false})
            })
            let call2 = cc.callFunc((sender: any)=>{
                this.parent.handCards[chair].getComponent("ZJHHandPoker").sendPoker();
                this.pokerPool.put(sender);
                if(i == totalPoker-1){
                    this.maskVisible(false);
                    this.parent.option.active = true;
                    this.deskPoker.length = 0;
                }
            });
            let seq = cc.sequence(delay,call1,m2,call2);
            poker.runAction(seq);
        }
    }
    ...
    startAnimation(data:any){
        ...
        Notification.sendNotify(Event.EVENT_MUSIC_EFFECT,{tag:4,mid:3,isLoop:false})
    }
    ...
    compareAnimation(data: any){
        ...
        let func = (x: number,d: any,url: string, Index:number)=>{
            ...
            list[2].spriteFrame = d.head.getComponent(cc.Sprite).spriteFrame;
            if(Index === 0){//我自己参与比牌
        ...
        let volume = cc.callFunc(()=>{Notification.sendNotify(Event.EVENT_MUSIC_EFFECT,{tag:4,mid:5,isLoop:false})})//比牌音效
        this.nodes1[0].stopAllActions();
        this.nodes2[0].stopAllActions();
        this.nodes1[0].runAction(cc.sequence(mb1,volume,mb3,mb4)); 
    ...
    playerWin(){
        this.youwin.node.active = true;
        this.youwin.setAnimation(1,"zjh_youwin",false);
        Notification.sendNotify(Event.EVENT_MUSIC_EFFECT,{tag:4,mid:1,isLoop:false})//胜利音效
    }

    startCount(time: number){
        this.hideNotice()
        this.notice_start.active = true;
        this.endTime  = time*1000+new Date().getTime();
        this.countFlag = true;
    ...
    AllInFire(){
        Notification.sendNotify(Event.EVENT_MUSIC_EFFECT,{tag:4,mid:4,isLoop:true})//全押音乐
    ...
    update(dt){
        ...
        let deltaTime = Math.ceil((this.endTime - currentTime)/1000)
        this.notice_start.getChildByName("count").getComponent(cc.Label).string = String(deltaTime)
        if(deltaTime <= 0){
            this.stopCount();
        }
    }
