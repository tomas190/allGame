...
export default class ZJHOption extends cc.Component {
    ...
    bet(event:any,id:number){
        ...
        switch(index)
        {
            case 0://全押
                ...
                break;
            case 1://比牌
                if(money < this.minBet){
                    NoticeMgr.showToast(baseDef.LOCAL_TXT.MONEY_NOT_ENOUGH);
                    return;
                }
    ...
    followBet(){
        if(Myself.getMoney() < this.minBet){//钱不够取消自动跟
            this.btns[7].getChildByName("checkMark").active = false;
            this.keepBet = false;
            this.updateBtn(this.round,this.myTurn,Desk.getUserNumInGame() == 2,this.anyOneAllIn,this.minBet,true);
            return;  
        }
