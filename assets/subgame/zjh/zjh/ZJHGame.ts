...
import ZJHNotifacition = require("./ZJHnet/ZJHNotification")
...
export default class ZJHGame extends cc.Component {
    ...
    @property
    compareChair:number = 0;

    @property
    isInGame:boolean = false;//自己是否在游戏中

    @property
    basePoint:number = 0;//房间底注
    ...
    ZindexInit(){
        this.rule.zIndex = BaseDef.ZOrder.HIGH;
        this.emoji.zIndex = BaseDef.ZOrder.HIGH;
    }
    ...
    onEnterGame(data: any){
        ...
        this.basePoint =  roominfo.basepoint;
        this.difen.string = "底注:" + this.basePoint;
        ...
        switch(data.currstate){
            ...
            case 20: case 21: case 22://已经开始玩游戏
                ...
                let chips_ = this.splitChips(data.total,true);//设置桌面筹码
                ...
                this.someOneAllIn = data.allinchair != -1;
                if(this.someOneAllIn) this.aniView.getComponent("ZJHAnimationView").AllInFire();
                if(this.myIndex == data.allinchair) this.option.active = false;//我自己全押
                this.turnBet({chair:data.currbetchair,round:roominfo.currround,minmoney:data.MinXZMoney,cmpdchair:data.comparedchair},roominfo.thinktime,data.countdown);
                break;
            case 29:{//结束
                break;
            }        
        ...
        for(let i=0; i<data.length; ++i){
            ...
            if(stage >= 20 && stage <= 22){//玩牌阶段
                ...
                if(data[i].isingame){//在游戏中
                    ...
                    handCard.setBets(data[i].BetMoney);//设置下注数量
                    if(index == 0){
                    ...
                    if(index == 0) {
                        this.aniView.getComponent("ZJHAnimationView").waitNotice(2);//提示等待下一局
    ...
    setBanker(chair: number){
        ...
        this.banker.zIndex = BaseDef.ZOrder.LOW;
    ...
    dealCard(data: any){
        this.isInGame = true;
        this.aniView.getComponent("ZJHAnimationView").startAnimation(data);
    }

    playerWin(chair:number){
        let index = this.getViewIndex(chair);
        ZJHNotifacition.sendNotify(EVENT.EVENT_MUSIC_EFFECT,{tag:4,mid:6,isLoop:false})
        this.handCards[index].getComponent("ZJHHandPoker").win();
        if(index == 0 && this.gamestage === GameState.ONGOING) this.option.active = true;//我比牌胜利显示操作按钮
    }
    ...
    getEmoji(id:number, chair:number){
        ...
        let pos = this.getPosByChair(index);
        emoji.active = true;
        emoji.zIndex = BaseDef.ZOrder.MIDDLE;
    ...
    sendEmoji(sendChair:number, reveiveChair:number, id:number){
        ...
        let call = cc.callFunc(()=>{
            ZJHNotifacition.sendNotify(EVENT.EVENT_MUSIC_EFFECT,{tag:3,mid:id,isLoop:false})
            emoji.getComponent(cc.Animation).play();
        });
        if(id === 3){//丟拖鞋
    ...
    leaveDesk(){
        ...
        this.option.getComponent("ZJHOption").onExit();
        gameController.switchHall();
        cc.audioEngine.stopAll();//停止所有音效
    }

    btnEvent(event: any,id: string){
        switch(id){
            case "1":
                    if(this.gamestage == GameState.ONGOING && this.isInGame){
                        NoticeMgr.openDialog({str:BaseDef.LOCAL_TXT.QUIt_TXT,callfunc:this.leaveDesk,obj:this,close:true})
                        return;
                    }
                    this.leaveDesk();
                break;
            case "2":{
                    ...
                    let m2 = cc.moveTo(0.2,cc.v2(posx,0));
                    this.rule.runAction(m2);
                }
                break;
            case "3":
    ...
    splitChips(val:number,flag: boolean){
        let len = this.chipConfig.length;
        let arr = {};
        let s = [];
        if(flag){
            let userNum = Desk.getUserNum();
            val-=this.basePoint*userNum;
            for(let i=0; i<userNum; ++i){
                s.push(this.basePoint);
            }
        }
        if(val>0){
    ...
    betting(data: any, type?: number){
        ...
        if(index == 0 && data.residuemoney) MySelf.setMoney(data.residuemoney);//每一笔下注更新保存的金币数量
    }
    ...
    bbb(index: number,val:number){
        ...
        this.players[index].getComponent("ZJHPlayer").stopCountDown();//先停止下注的人的倒计时
        let pos = this.getPosByChair(index);
        let chips_ = this.splitChips(val,false);
        this.chipFly(chips_,pos);
    }

    settle(data: any){
        ZJHNotifacition.sendNotify(EVENT.EVENT_MUSIC_STOPEFFECT,{tag:4,mid:4})
        ...
        let perChips = ~~(this.deskChip.length/winners.length)//多个赢家平分筹码
    ...
    turnBet(data: any,totalTime: number,remainTime: number){
        ...
        /*----清除上次比牌-----*/
        this.compareChair = data.cmpdchair
        this.currentChair = this.getViewIndex(data.chair);;
        ...
        if(this.compareChair == 99){
            msgSender.playerCmpWithOther(this.compareChair);//自动发起比牌
            return;
        }
    ...
    initHandCardsPos(){
        for(let i=1; i<this.handCards.length; ++i){
            ...
            let size2 = this.handCards[i].getChildByName("ani_zjh_win").getContentSize();
    ...
    compareReq(){
        ...
        let index = this.getViewIndex(this.compareChair);
    ...
    AllIn(data: any){
        ...
        let pos = this.getPosByChair(index);
        let chips_ = this.splitChips(val,false);
        ZJHNotifacition.sendNotify(EVENT.EVENT_MUSIC_EFFECT,{tag:4,mid:2,isLoop:false})//丢一堆筹码音效
        this.chipFly(chips_,pos);
        //播全压动画
        if(data.win == -1) {
            this.aniView.getComponent("ZJHAnimationView").AllInFire();
        }else{ //第二个人全押比牌
            this.aniView.getComponent("ZJHAnimationView").compareAnimation({win:data.win,lose:data.lose});
        }
    }
    ...
    startCompare(data:any){
        this.mask.active = false;
         //重置比牌玩家层级
        let index2 = this.getViewIndex(this.compareChair);
        this.handCards[index2].getComponent("ZJHHandPoker").offClick();
        this.handCards[index2].zIndex = BaseDef.ZOrder.BOTTOM;
        this.players[index2].zIndex = BaseDef.ZOrder.BOTTOM;
        ...
        if(data.betmoney) {
            this.bbb(index,data.betmoney);//丢筹码
            this.setPoolValue(data.betmoney);
        }
        if(data.win == this.myIndex || data.lose == this.myIndex) this.option.active = false;//我参与比牌隐藏操作按钮
    ...
    onGameExit(){
        ...
        for(let i=0; i<this.deskChip.length; ++i){
