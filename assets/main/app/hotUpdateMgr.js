...
let hotUpdateMgr = {
    ...
    init(data) {
        ...
        this._mainmanifest = data.mainmanifest;
        this._hallmanifest = data.hallmanifest;
        this._paymanifest = data.paymanifest;
        this._proxymanifest = data.proxymanifest;
        this._IMmanifest = data.IMmanifest;
    },
    ...
    checkFile(checkManifest) {
        if (
            this.data.subname == "main"
            || this.data.subname == "hall"
            || this.data.subname == "pay"
            || this.data.subname == "proxy"
            || this.data.subname == "IM"
            || this.data.subname == ""
        ) {
    ...
    m_renameManifest() {
        ...
        if (jsb.fileUtils.isFileExist(file)) {
            ...
            if (mod == "main" || mod == "") {
                ...
                gHandler.appGlobal.version = mstr.version
            } else if (mod == "hall") {
                gHandler.localStorage.globalSet(gHandler.appGlobal.hallVersionKey, mstr.version)
                gHandler.appGlobal.hallVersion = mstr.version
            } else if (mod == "pay") {
                gHandler.localStorage.globalSet(gHandler.appGlobal.payVersionKey, mstr.version)
                gHandler.appGlobal.payVersion = mstr.version
            } else if (mod == "proxy") {
                gHandler.localStorage.globalSet(gHandler.appGlobal.proxyVersionKey, mstr.version)
                gHandler.appGlobal.proxyVersion = mstr.version
            } else if (mod == "IM") {
                gHandler.localStorage.globalSet(gHandler.appGlobal.IMVersionKey, mstr.version)
                gHandler.appGlobal.IMVersion = mstr.version
            } else {
            ...
            if (this.data.subname == "main" || this.data.subname == "") {
                gHandler.localStorage.globalSet(gHandler.appGlobal.versionKey, this._downV)
                gHandler.appGlobal.version = this._downV
            } else if (this.data.subname == "hall") {
                gHandler.localStorage.globalSet(gHandler.appGlobal.hallVersionKey, this._downV)
                gHandler.appGlobal.hallVersion = this._downV
            } else if (this.data.subname == "pay") {
                gHandler.localStorage.globalSet(gHandler.appGlobal.payVersionKey, this._downV)
                gHandler.appGlobal.payVersion = this._downV
            } else if (this.data.subname == "proxy") {
                gHandler.localStorage.globalSet(gHandler.appGlobal.proxyVersionKey, this._downV)
                gHandler.appGlobal.proxyVersion = this._downV
            } else if (this.data.subname == "IM") {
                gHandler.localStorage.globalSet(gHandler.appGlobal.IMVersionKey, this._downV)
                gHandler.appGlobal.IMVersion = this._downV
            } else {
                gHandler.localStorage.set(this.data.subname, "versionKey", this._downV)
            }
        }
    ...
    _loadCustomManifest: function (subname, verstr) {
        ...
        if (jsb.fileUtils.isFileExist(localpath)) {
            ...
            if (subname == 'main' && this.manifestData.mainmanifest) {
                var nurl = this.manifestData.mainmanifest.nativeUrl;
                this.log("native url", nurl)
                if (cc.loader.md5Pipe) {
                    nurl = cc.loader.md5Pipe.transformURL(nurl);
                }
                this._am.loadLocalManifest(nurl);
            } else if (subname == 'hall' && this._hallmanifest) {
                var nurl = this._hallmanifest.nativeUrl;
                this.log("hall native url", nurl)
                if (cc.loader.md5Pipe) {
                    nurl = cc.loader.md5Pipe.transformURL(nurl);
                }
                this._am.loadLocalManifest(nurl);
            } else if (subname == 'pay' && this._paymanifest) {
                var nurl = this._paymanifest.nativeUrl;
                this.log("pay native url", nurl)
                if (cc.loader.md5Pipe) {
                    nurl = cc.loader.md5Pipe.transformURL(nurl);
                }
                this._am.loadLocalManifest(nurl);
            } else if (subname == 'proxy' && this._proxymanifest) {
                var nurl = this._proxymanifest.nativeUrl;
                this.log("proxy native url", nurl)
                if (cc.loader.md5Pipe) {
                    nurl = cc.loader.md5Pipe.transformURL(nurl);
                }
                this._am.loadLocalManifest(nurl);
            } else if (subname == 'IM' && this._IMmanifest) {
                var nurl = this._IMmanifest.nativeUrl;
                this.log("IM native url", nurl)
                if (cc.loader.md5Pipe) {
                    nurl = cc.loader.md5Pipe.transformURL(nurl);
                }
                this._am.loadLocalManifest(nurl);
            } else {
                let url = gHandler.appGlobal.canHotServer + "/" + gHandler.appGlobal.hotupdatePath + "/" + verstr;
    ...
    checkUpdate: function (data) {
        ...
        let mainversion = gHandler.localStorage.globalGet(gHandler.appGlobal.versionKey)
        mainversion = mainversion ? mainversion : ''
        this.log('开始更新 version', data.subname, this.data.version, 'mainversion', mainversion)
        let verstr = ''
        if (this.data.subname == 'main' && this.data.remotev) {
        ...
        this._packageUrl = gHandler.appGlobal.canHotServer + "/" + gHandler.appGlobal.hotupdatePath + "/" + verstr
