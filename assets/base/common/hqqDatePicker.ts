 ...
 */
 ...
 @ccclass
 export default class hqqDatePicker extends cc.Component{
     @property(cc.Label)
     lbYearMonth:cc.Label = null;

     @property(cc.Label)
     lbYearMonth2:cc.Label = null;

     @property(cc.Node)
     ndDays:cc.Node = null;

     @property(cc.Node)
     ndDays2:cc.Node = null;
     ...
     pfgListDay:Array<cc.Node> = [];
     pfgListDay2:Array<cc.Node> = [];
    ...
    updateDate () {
        ...
        for (let i = 0; i < this.pfgListDay2.length; ++i) {
            ...
            if (i < totalDays2) {
                ...
                let row = Math.floor(index / 7);
                let col = index % 7;
                let x = -(this.ndDays2.width - node.width) * 0.5 + col * node.width;
                let y = (this.ndDays2.height - node.height) * 0.5 - row * node.height;
                node.setPosition(x, y);
                let script = node.getComponent("hqqItemDay");
