...
export default class hqqPersonalCenter extends cc.Component {
    @property(cc.Prefab)
    selectDatePrefab:cc.Prefab = null;
     ...
     * 投注总额
     */
    tzcount:cc.Label = null;
    /**
     * 派彩总额
     */
    pccount:cc.Label = null;
    /**
     * 订单数
     */
    ddcount:cc.Label = null;
    /**
     * 盈利总额
     */
    income:cc.Label = null;

    /**
     * 盈利概括查询天数选单列表
     */
    YLGKDayList:cc.Node = null;

    /**
     * 盈利概括查询天数LABEL
     */
    YLGKDayLabel:cc.Label = null;
    ...
    /**投注记录最大页数文本*/
    TZJLMaxPageLabel:cc.Label = null;
    
    /**投注记录游戏分类清单*/
    YXFLList:Array<Array<YXFL>> = [[],[],[],[],[],[]];

    /**投注记录选择日期文本 */
    TZJLSelectDateLabel:cc.Label = null;

    /**投注记录选择日期节点 */
    TZJLSelectDateNode:cc.Node = null;

    /**投注记录选择日期节点 */
    TZJLSelectDatePanel:cc.Node = null;

    /**投注记录选择游戏名称文本 */
    TZJLSelectGameLabel:cc.Label = null;
    /**投注记录选择游戏名称清单节点 */
    TZJLSelectGameList:cc.Node = null;
    /**投注记录选择游戏名称 */
    TZJLSelectGameID:string = "";

    /**投注记录选择排序文本 */
    TZJLSelectSortLabel:cc.Label = null;
    /**投注记录选择排序文本清单节点 */
    TZJLSelectSortList:cc.Node = null;
    ...
    BZYJ:number = 0;
    /**本周业绩文本 */
    BZYJLabel:cc.Label = null;
    
    /**本周充值 */
    BZCZ:number = 0;
    /**本周充值文本 */
    BZCZLabel:cc.Label = null;
    
    /**本周提现 */
    BZTX:number = 0;
    /**本周提现文本 */
    BZTXLabel:cc.Label = null;
    
    /**棋牌待遇 */
    QPDY:number = 200;
    /**棋牌待遇文本 */
    QPDYLabel:cc.Label = null;

    /**电子待遇 */
    DZDY:number = 130;
    /**电子待遇文本 */
    DZDYLabel:cc.Label = null;
    ...
    onMenuClick(toggle,customEventData){
        cc.find("checkmark/dating_menu_btn_selected",toggle.node).getComponent(sp.Skeleton).setAnimation(0,"animation",true);
        if( Number(customEventData) === 1 ){
            ...
            cc.find("personalcenter/btncontainer/tzjl/checkmark/basicInfo_bg/tabScrollView/view/tabToggleContainer").children[0].getComponent(cc.Toggle).check();
            ...
            cc.find("personalcenter/btncontainer/ylgk/checkmark/basicInfo_bg/tabScrollView/view/tabToggleContainer").children[0].getComponent(cc.Toggle).check();
    ...
    onTZJLTabClick(toggle,customEventData){
        ...
        for(let i = 1; i <= 5; i++){
            let skeleton = cc.find("personalcenter/btncontainer/tzjl/checkmark/basicInfo_bg/tabScrollView/view/tabToggleContainer/toggle" + i + "/Background").getComponent(sp.Skeleton);
            if(cc.isValid(skeleton)){
                skeleton.setAnimation(0,"animation",true);
            }
        }
        if(cc.isValid(toggle)){
            cc.find("checkmark/setting_tabselect",toggle.node).getComponent(sp.Skeleton).setAnimation(0,"animation",true);
        }
    }
    ...
    onTZJLSelectDateClick(event,customEventData){
        if(!cc.isValid(this.TZJLSelectDatePanel)){
            ...
            this.node.addChild(this.TZJLSelectDatePanel);
        }
    }
                ...
                for(let i = 0; i < this.YXFLList[this.TZJLTabIndex].length;i++){
                    this.scheduleOnce(()=>{
                        ...
                        if(tempgamebtn.clickEvents.length === 0 ){
                            let gameclickEventHandler = new cc.Component.EventHandler();
    ...
    onYLGKDayClick(event:cc.Event.EventTouch,customEventData){
    ...
    onYLGKTabClick(toggle,customEventData){
        ...
        for(let i = 1; i <= 5; i++){
            let skeleton = cc.find("personalcenter/btncontainer/ylgk/checkmark/basicInfo_bg/tabScrollView/view/tabToggleContainer/toggle" + i + "/Background").getComponent(sp.Skeleton);
            if(cc.isValid(skeleton)){
                skeleton.setAnimation(0,"animation",true);
            }
        }
