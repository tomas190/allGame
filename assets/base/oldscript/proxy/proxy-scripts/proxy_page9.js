...
cc.Class({
    ...
    page9setItemData(data) {
        ...
        if (gHandler.app.pinpai != 'ninetwo') {
            ...
            } else if (gHandler.app.pinpai == "huangshi" && Database.page9_plaumont == '100元。') {
                btn.active = false;
            } else {
                if (gHandler.app.pinpai == "juding" || gHandler.app.pinpai == "huaxing"|| gHandler.app.pinpai == "tianqi") {
                    btn.on("touchend", () => {
                        cc.log('juding111111');
                        //音效
                        Database.clickSound(Database.hall_sound)
            ...
            cc.find("label2", newItem).getComponent(cc.Label).string = times


            //投注额
            if (data.user_bet_total) {
                cc.find("label3", newItem).getComponent(cc.Label).string = Math.floor(parseFloat(data.user_bet_total) * 100) / 100 + ''
            } else {
                cc.find("label3", newItem).getComponent(cc.Label).string = '0'
            }

            //充值
            if (data.user_top_up_total) {
                cc.find("label4", newItem).getComponent(cc.Label).string = Math.floor(parseFloat(data.user_top_up_total) * 100) / 100 + ''
            } else {
                cc.find("label4", newItem).getComponent(cc.Label).string = '0'
            }
