...
cc.Class({
    ...
    init: function (num) {
        ...
        token = commonVal.token;
        var self = this;
        cc.resources.load(`/proxy/language/CN/title_${num}`, cc.SpriteFrame, function (err, spriteFrame) {
            self.node
    ...
    initMycustomers: function (id, count) {
        ...
        closeEventHandler.target = cc.find("Canvas/baseView/home/page1"); // 这个 node 节点是你的事件处理代码组件所属的节点
        closeEventHandler.component = "proxy-page1"; // 这个是代码文件名
        ...
        if (parseInt(count) !== 1) {
            ...
            switch (parseInt(count)) {
                case 2:
                    cc.resources.load("/proxy/language/CN/proxy-title1", cc.SpriteFrame, function (
                        err,
                        spriteFrame,
                    ) {
                        self.node
                    ...
                    break;
                case 3:
                    cc.resources.load("/proxy/language/CN/proxy-title2", cc.SpriteFrame, function (
                        err,
                        spriteFrame,
                    ) {
                        self.node
                    ...
                    break;
                default:
                    cc.resources.load("/proxy/language/CN/proxy-title4", cc.SpriteFrame, function (
                        err,
                        spriteFrame,
                    ) {
                        self.node
            ...
            cc.log("打开了我的客户！", count, account_name, id);
        }
    ...
    agentDetails: function (num) {
        ...
        function callback(scrollView) {
           ...
           cc.log("在滑动！！！1111", scrollView.getScrollOffset().y);
                                    ...
                                    gridArray.forEach((item) => {
                                        res.msg.forEach((ele) => {
                                            if (item.id === ele.proxy_user_id) {
                                                //因为res.msg只包含带有下级的客户，遍历gridArrayh和res.msg,进行比较
                                                item.label4 += ele.statement_income * 0.35;
                                            }
                                        });
                                    });
                                }
                    ...
                    Promise.all([promise1, promise2, promise3]).then((values) => {
                        ...
                        gridArray.forEach((item) => {
                            ...
                            let action = {
                                ...
                                isfirstlevel: count === 1 ? true : false,
                            };
